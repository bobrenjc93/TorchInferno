    ...
    def _run_queued_stream_group(self, group: list[_QueuedGeneration]) -> None:
        ...
        with _tensor_parallel_symm_mem_allreduce_scope(
            ...
            if use_prompt_list_batch:
                ...
                try:
                    # Mid-batch early restart: break the decode loop as soon as
                    # ANY row hits a stop token, so the batch worker can restart
                    # with remaining live rows + any new arrivals from the queue.
                    # TP-safe: both ranks see the same tokens (NCCL-synchronized)
                    # and the same active[] state, so both break at the same step
                    # WITHOUT any extra dist.broadcast (which would desync NCCL).
                    # The callback simply returns True unconditionally -- the
                    # condition 'final_active_rows < active_rows_before' in the
                    # generator already gates when it fires.
        ...
        *,
        ...
        try:
            for step in range(1, max_tokens):
                ...
                yield step_tokens
                # Mid-batch early restart: if ANY row finished this step (went
                # from active to inactive, via stop token or max_tokens), break
                # the decode loop so the batch worker can restart with the
                # remaining live rows + new arrivals. TP-safe: active[] is
                # derived from NCCL-synchronized model outputs (both ranks see
                # the same tokens), and early_restart_check broadcasts the
                # primary's queue status to workers so all ranks break together.
                if (
                    early_restart_check is not None
                    and final_active_rows < active_rows_before

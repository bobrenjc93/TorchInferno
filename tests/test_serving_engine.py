...
def test_continuous_batch_engine_exact_prompt_reuses_generated_prefix_chain(
    monkeypatch,
) -> None:
    monkeypatch.setenv("TORCHINFERNO_CONTINUOUS_GENERATED_PREFIX_CACHE", "1")
    prompt = tuple(range(1, 18))
    ...
    engine.submit_online(ServingRequest("late-a", prompt, 3, arrival_step=0))
    assert [(event.request_id, event.token, event.finished) for event in engine.step_online()] == [
        ("late-a", 18, False)
    ]
    assert [(event.request_id, event.token, event.finished) for event in engine.step_online()] == [
    ...
    decode_calls = engine.stats.decode_model_calls
    assert engine.stats.generated_prefix_store_requests == 2

    engine.submit_online(ServingRequest("late-b", prompt, 3, arrival_step=0))
    events = engine.step_online()
    ...
    assert not engine.has_online_work()
    assert engine.stats.decode_model_calls == decode_calls
    assert engine.stats.generated_prefix_reuse_requests == 2
    assert engine.stats.generated_prefix_reuse_tokens == (len(prompt) + 1) + (len(prompt) + 2)


def test_continuous_batch_engine_exact_prompt_elides_deep_cached_stop_event(
    monkeypatch,
) -> None:
    monkeypatch.setenv("TORCHINFERNO_CONTINUOUS_GENERATED_PREFIX_CACHE", "1")
    prompt = tuple(range(1, 18))
    ...
    engine.submit_online(
        ServingRequest("late-b", prompt, 3, arrival_step=0, eos_token_id=eos_token_id)
    )
    events = engine.step_online()
